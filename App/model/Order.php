...
//put your code here
    ...
    public function getOrderProducts($id){
        $orderProducts = $this->db->select('order_products','*',"order_id = $id");
    ...
    public function getUsers() {
        $myUsers = $this->db->select('users', '*' ,"role_id = 2");
        return $myUsers;
    }
...
public function myOrderChecks($fDate,$lDate){
    $myOrders;
if(!empty($fDate) && !empty($lDate)){
    $myOrders = $this->db->select('orders','*',"date BETWEEN '$fDate' AND '$lDate';");
} else {
...
{
    //user_id = $uid
    // $sum = $this->db->getTotal('orders','sum(total_price) as sum',null,"user_id");
     $sum = $this->db->getTotal('orders','sum(total_price) as sum' ,"user_id=$uid" ,"user_id");
    // var_dump($myOrders);
    return $sum;
}
 public function getUser($id) {
        $user = $this->db->selectById("users", "*", "id=$id")['resultset'];
